...
with open('test.js','r',encoding='utf-8') as f:
    ctx = execjs.compile(f.read())
    result = ctx.call('de_crawl_data',a,key)
# 返回数组，通过bytes hex将数组转字符串，再通过fromhex转16进制字节符号
res_data = bytes.fromhex(bytes(result['data']).hex())
print(res_data)
with open('result.txt','wb') as f:
    f.write(bytes.fromhex(bytes(result['data']).hex()))