...
# print(bytes(res.text, encoding='UTF-8'))
# print(len(binascii.b2a_hex(res.text.encode())))
# print(binascii.b2a_hex(res.text.encode()))
# a = ','.join([str(int(i,16)) for i in binascii.hexlify(res.text.encode(),'-').decode().split('-')])
# with open("resdata.json",'w')as f:
#     f.write(a)
with open('test.js','r',encoding='utf-8') as f:
    ctx = execjs.compile(f.read())
    result = ctx.call('de_crawl_data',c,key)
# print(result)
