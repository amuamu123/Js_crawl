...
with open('test.js','r',encoding='utf-8') as f:
    ...
    c_key =  bytes.fromhex(bytes(ctx.call('base64ToArrayBuffer',key)['data']).hex())
...
print(len(res.text))
# print(binascii.b2a_hex(res.text.encode()).decode())
# print(binascii.b2a_hex(res.text.encode()).decode())
with open('./ibox_crawl/lin.txt','w') as f:
    f.write(binascii.b2a_hex(res.text.encode()).decode())
with open('test.js','r',encoding='utf-8') as f:
    ctx = execjs.compile(f.read())
    result = ctx.call('de_crawl_data',binascii.b2a_hex(res.text.encode()).decode(),key)
print(result)